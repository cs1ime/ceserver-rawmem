...
#define COMPRESS_BLOCKSIZE (64 * 1024)
				...
				sender.push_data(&uncompressedSize, sizeof(uncompressedSize)); // followed by the compressed size
				sender.push_data(&compressedSize, sizeof(compressedSize));	   // the compressed data follows
				for (i = 0; i <= currentBlock; i++)
				{
					...
					{
						sender.push_data(compressedBlocks[i], COMPRESS_BLOCKSIZE - strm.avail_out); // last one, flush
						sender.send(0);
					}
			...
			{
				sender.push_data(o, sizeof(CeReadProcessMemoryOutput) + o->read);
				sender.send(0);
			}
	...
	case CMD_VIRTUALQUERYEX:
	{
		...
		if (r > 0)
		{
			...
			{
				datasender alldata(currentsocket);
				alldata.push_data(&o, sizeof(o));
				{
					uint8_t size = strlen(mapsline);
					alldata.push_data(&size, sizeof(size));
					alldata.push_data(mapsline, size);
					alldata.send(0);
				}
			}
		}
...
{
	cxxopts::Options options("ceserver-rawmem", "ceserver for raw memory dump file");
	options.add_options()("f,file", "Raw memory dump file name", cxxopts::value<std::string>())("p,port", "CEServer port", cxxopts::value<uint16_t>()->default_value("8997"));
	("h,help", "Show help");
	...
	{
		std::cout << "please specify a file through use -f paramter" << std::endl;
		std::cout << options.help() << std::endl;
		return 1;
	}
	...
	std::cout << "rawmemfile = " << rawmemfile.c_str() << std::endl;
