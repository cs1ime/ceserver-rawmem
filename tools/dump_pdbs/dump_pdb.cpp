...
int main(int argc, char *argv[]) {
  ...
  if (!strcmp(argv[1], "struct")) {
    ...
    for (auto &&[name, fields] : structures) {
      std::vector<std::pair<std::string, field_info>> sortedVec(fields.begin(),
                                                                fields.end());
      ...
      for (auto &&[field_name, field_info] : sortedVec) {
        std::cout << "\t"
                  ...
                  << field_info.offset << " */\t" << field_info.type_name << " "
                  << field_name;
        if (field_info.is_bitfield) {
          std::cout << std::dec;
          std::cout << " : " << field_info.bitfield_length;
          std::cout << "  /* " << field_info.bitfield_offset << ":"
                    << field_info.bitfield_offset + field_info.bitfield_length -
                           1
                    << " */";
          std::cout << std::hex;
        }
