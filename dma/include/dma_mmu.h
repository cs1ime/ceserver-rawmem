...
public:
    mmu_initializer(physaddr _dtb,u64 _pte_base,
		...
		read_physical_memory(_read_physical_memory),
    ...
    mmu_initializer(const mmu_initializer& _m)
    {
		...
		this->read_physical_memory=_m.read_physical_memory;
		this->write_physical_memory=_m.write_physical_memory;
    }
...
public:
	...
	bool chkvirt(u64 virt) {
		return virt2phys(virt) != D_BADPHYSADDR;
	}
	bool chkvirt(ptr virt) {
		return chkvirt((u64)virt);
	}
	...
	template <typename T>
	bool w(u64 virt, T data) {
		return write_virt(virt, &data, sizeof(T));
	};
	template <typename T>
	bool w(ptr virt, T data) {
...
public:
	...
	void set_prev_page(u64 prev_page) {
		prev_page1 = prev_page;
	}
	void set_using() {
		is_using1 = true;
	}
...
public:
    ...
    int memsch(uptr virt, u64 sz, const char* pattern,std::function<bool(u64 pos)>callback);
    u64 scanmem(u64 begin, u64 end, const char* pattern);
    int scanmem(u64 begin, u64 end, const char* pattern,std::function<bool(u64 pos)>callback);
	
	u64 enummem(uptr start_va, uptr end_va, std::function<bool(u64 start, u64 sz, u64 rwx)> callback);
