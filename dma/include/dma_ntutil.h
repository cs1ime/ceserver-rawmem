...
public:
	ntfuncs(const mmu_initializer& _m,u64 ntos,std::shared_ptr<dma_symbol_factory> symbol_factor=nullptr);
	~ntfuncs();
	mmu*
	getmmu() {
		return m_mmu;
    ...
    //returns true to continue enumerating
	int enumdrv(std::function<bool(u64 drvbase, u64 drvsize, pwstr drvname)> callback);
	...
	u64 GetPeb(u64 eprocess) {
		return m_mmu->r<u64>(offset_Peb + eprocess);
	}
...
private:
	...
	int BalancedNodeTraverse
	(u64 Node, std::function<bool(u64 Node)> callback);
public:
	process(ntfuncs* ntf, const mmu_initializer& _m, u64 eprocess, int pid) : mmu(_m) {
		this->ntf = ntf;
	...
	int enumvad(std::function<bool(u64 base, u64 size, u64 prot,u64 type)> callback);
