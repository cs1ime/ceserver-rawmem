...
ntfunc_creator::try_create()
{
	...
	if (pte_base == 0)
		return nullptr;
	g_sysmmu = std::make_shared<mmu>(mmu_initializer(root_tb, pte_base, read_physical_memory, write_physical_memory));
	...
	g_ptebase = pte_base;
	g_pfnbase = pfnbase;
	mmu_initializer _m(root_tb, pte_base, read_physical_memory, read_physical_memory);
...
physaddr ntfunc_creator::mmu_get_pa_from_dtb(physaddr dtb, uptr va)
{
	...
	read_physical_memory((page.page_frame_number << 12) + vva.pdpte_index * 8, (pbyte)&page.all, 8);
	if (page.present == 0)
		return badphysaddr;
	if (page.large_page)
	{
	...
	read_physical_memory((page.page_frame_number << 12) + vva.pde_index * 8, (pbyte)&page.all, 8);
	if (page.present == 0)
		return badphysaddr;
	if (page.large_page)
	{
	...
	read_physical_memory((page.page_frame_number << 12) + vva.pte_index * 8, (pbyte)&page.all, 8);
	if (page.present == 0)
		return badphysaddr;
...
{
	auto dma_check_vaild_buildnumber = [&](int buildnumber) -> bool
	{
	...
	auto dma_check_root_tb_judgement2 = [&](u8 *pb, physaddr pa) -> bool
	{
		...
		if (!dma_check_vaild_buildnumber(buildnumber))
			return false;
	...
	if (dma_check_root_tb_judgement1(pb, pa) && dma_check_root_tb_judgement2(pb, pa))
		return true;
	return false;
}
...
u64 ntfunc_creator::dma_find_ntoskrnl(physaddr dtb)
{
	...
	u64 ntos = 0;
	g_sysmmu->enummem(start_va.all, end_va.all, [&](u64 start, u64 sz, u64 rwx) -> bool
					  {
		printf("%llX %llX\n", start, sz);
			...
			if (g_sysmmu->r<WORD>(start + i * 0x1000) == 0x5A4D) {
				...
				if (magic == 0x00004550) {
					...
					IMAGE_SECTION_HEADER* ish =
						(IMAGE_SECTION_HEADER*)(((pbyte)&inh->OptionalHeader) + g_sysmmu->r<WORD>(&inh->FileHeader.SizeOfOptionalHeader));
					
					WORD sectcnt = g_sysmmu->r<WORD>(&inh->FileHeader.NumberOfSections);
					for (int j = 0; j < sectcnt; j++) {
						IMAGE_SECTION_HEADER* curish = &ish[j];
						char sectname[9];
						memset(sectname, 0, 9);
						*(u64*)(sectname) = g_sysmmu->r<u64>(&curish->Name);
						if (*(DWORD*)sectname == 0x4C4F4F50 && *(DWORD*)(sectname + 4) == 0x45444F43) {
							printf("finded!\n");
							ntos = hdr;
							return false;
						}
