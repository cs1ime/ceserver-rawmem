...
#define fetch_base_data                                                                  \
    ...
    {                                                                                    \
        ish.push_back(r<IMAGE_SECTION_HEADER>(pish + sizeof(IMAGE_SECTION_HEADER) * i)); \
    }
...
u64 peMemory64::scansect(const std::string &sectname, const std::string &pattern)
{
    fetch_base_data;
...
{
    fetch_base_data;
    DWORD va = inh.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION].VirtualAddress;
    ...
    if (va)
    {
        ...
        {
            IMAGE_RUNTIME_FUNCTION_ENTRY func = r<decltype(func)>(base + va + i * sizeof(decltype(func)));
...
{
    fetch_base_data;
    DWORD rva = inh.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].VirtualAddress;
    DWORD size = inh.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].Size;
    if (rva)
    {
        ...
        while (through_size < size)
        {
            ...
            if (dbg.Type == IMAGE_DEBUG_TYPE_CODEVIEW)
            {
                ...
                if (
                    read_string(
                        (u64)(&((praw)->pdb_file_name)),
                        buffer,
                        1024))
                {
                    return std::make_tuple<std::string, std::string, uint32_t>(std::string(buffer), ss.str(), std::move(raw.age));
                }
            }
