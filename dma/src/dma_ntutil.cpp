...
#define GetBitFieldValue(v, a, b) (((uint64_t)v) << ((sizeof(uint64_t) * 8) - (b + 1)) >> ((sizeof(uint64_t) * 8) - (b + 1)) >> (a))
...
ULONG64 FindPspCidTable(ntfuncs *funcs)
{
	...
	if (BuildNumber < WIN10)
	{
		...
		ULONG64 pos = funcs->kscansect(ntos, xs("PAGE"), xs("48 8B D1 48 8B 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 45"));
		...
		{
			pos = funcs->kscansect(ntos, xs("PAGE"), xs("00 00 ?? ?? ?? 48 8B D1 48 8B 0D ?? ?? ?? ?? E8"));
			if (pos)
			{
	...
	else if (BuildNumber >= WIN10 && BuildNumber <= WIN10_1607)
	{
		...
		ULONG64 pos = funcs->kscansect(ntos, xs("PAGE"), xs("E8 02 00 00 48 8B 0D ? ? ? ? ? ? ? ? E8"));
		...
		{
			pos = funcs->kscansect(ntos, xs("PAGE"), xs("30 06 00 00 48 8B 0D ?? ?? ?? ?? E8"));
			if (pos)
			{
				return parse(pos, 7, 11);
			}
			...
			{
				pos = funcs->kscansect(ntos, xs("PAGE"), xs("48 8B 05 ?? ?? ?? ?? ?? ?? ?? F7 C1"));
				if (pos)
				{
	...
	else if (BuildNumber >= WIN10_1703)
	{
		...
		ULONG64 pos = funcs->kscansect(ntos, xs("PAGE"), xs("E8 ?? ?? ?? ?? 48 8B 0D ?? ?? ?? ?? ?? ?? ?? ?? ?? 45 33 C9 45 33 C0 ?? ?? ?? E8"));
		if (pos)
		{
		...
		{
			pos = funcs->kscansect(ntos, xs("PAGE"), xs("00 00 48 8B 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 85 C0"));
			if (pos)
			{
...
{
	u64 ObReferenceObjectByPointer = thiz->ksym(thiz->GetNtoskrnl(), xs("ObReferenceObjectByPointer"));
	// 0F B6 05 ?? ?? ?? ?? 48 33 C8 48 8D 05 ?? ?? ?? ??
	u64 pos = thiz->kscanmem(ObReferenceObjectByPointer, 0x1000, xs("0F B6 05 ?? ?? ?? ?? 48 33 C8 48 8D 05 ?? ?? ?? ??"));
	if (pos == 0)
		return 0;
...
bool ntfuncs::initialize_without_symbol()
{
	...
	if (!PsProcessType)
		return false;
	offset_SectionBaseAddress = rr<u32>(ksym(ntos, xs("PsGetProcessSectionBaseAddress")) + 3);
	...
	if (!offset_DirectoryTableBase)
		return false;
	offset_ImageFileName = rr<u32>(ksym(ntos, xs("PsGetProcessImageFileName")) + 3);
	if (!offset_ImageFileName)
		return false;
	offset_ProcessExitTime = rr<u32>(kscanmem(ksym(ntos, xs("PsGetProcessExitTime")), 0x1000, xs("48 8B 80 ?? ?? ?? ?? C3")) + 3);
...
bool ntfuncs::initialize_with_symbol()
{
	...
	std::cout << "name: " << name << " guid: " << guid << " age: " << age << std::endl;
	...
	try
	{
		...
		offset_SectionBaseAddress = all_structures["_EPROCESS"]["SectionBaseAddress"].offset;
		offset_Peb = all_structures["_EPROCESS"]["Peb"].offset;
		offset_WoW64Process = all_structures["_EPROCESS"]["WoW64Process"].offset;
		offset_DirectoryTableBase = all_structures["_KPROCESS"]["DirectoryTableBase"].offset;
...
std::shared_ptr<std::map<std::string, pdbinfo>>
ntfuncs::allpdbs()
{
	return m_allpdbs;
}

bool ntfuncs::is_vaild()
{
	return m_valid;
}
ntfuncs::ntfuncs(const mmu_initializer &_m, u64 ntos, std::shared_ptr<dma_symbol_factory> symbol_factory)
...
u64 ntfuncs::ksym(uptr mod, const char *symname)
{
	...
	IMAGE_EXPORT_DIRECTORY *ied = (IMAGE_EXPORT_DIRECTORY *)((u64)mod + r(inh->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress));

	DWORD NumberOfNames = r(ied->NumberOfNames);
	for (int i = 0; i < NumberOfNames; i++)
	{
		USHORT index = rr<USHORT>(&((USHORT *)((u64)mod + r(ied->AddressOfNameOrdinals)))[i]);
		ULONG NameRVA = rr<ULONG>(&((ULONG *)((u64)mod + r(ied->AddressOfNames)))[i]);
		...
		{
			ULONG FunRVA = rr<ULONG>(&((ULONG *)((u64)mod + r(ied->AddressOfFunctions)))[index]);
			...
			// 	ULONG64 base = (ULONG64)drvbase((PCHAR)BoundImportModuleName);
			// 	if (base) {
...
ULONG64 EMU_ExpLookupHandleTableEntry(mmu *m_mmu, ULONG_PTR a1, ULONG64 a2)
{
	...
	if ((v3 & 3) != 0)
		return rr<ULONG64>(rr<ULONG64>(v3 + 8 * (v2 >> 19) - 2) + 8 * ((v2 >> 10) & 0x1FF)) + 4 * (v2 & 0x3FF);
	return v3 + 4 * v2;
}
u64 ntfuncs::lookupcid(u64 cid)
{
	ULONG64 Entry = EMU_ExpLookupHandleTableEntry(m_mmu, rr<u64>(PspCidTable), cid);
	if (Entry == 0)
		return 0;
	// DbgPrint("Entry:%p\n", Entry);
	if (Entry)
...
int ntfuncs::enumdrv(std::function<bool(u64 drvbase, u64 drvsize, pwstr drvname)> callback)
{
	...
	while (first != query)
	{
		...
		if (maxlen > 78)
			maxlen = 78;
		m_mmu->read_wstring((u64)r(ldr->BaseDllName.Buffer), (pwstr)drvname, maxlen);
...
u64 ntfuncs::drvbase(const char *name)
{
	...
	u64 result = 0;
	enumdrv(
		[&](u64 drvbase, u64 drvsize, pwstr drvname) -> bool
		{
...
int process::BalancedNodeTraverse(u64 Node, std::function<bool(u64 Node)> callback)
{
	auto pdb = sys()->allpdbs();
	auto &&ntos = (*pdb)["ntoskrnl.exe"];

	int offset_Children = ntos.structures["_RTL_BALANCED_NODE"]["Children"].offset;
	...
	int offset_ParentValue = ntos.structures["_RTL_BALANCED_NODE"]["ParentValue"].offset;
...
int process::enummod(std::function<bool(u64 base, u64 size, pwstr name)> callback)
{
	int mod_cnt = 0;
	...
	if (peb && m_mmu->chkvirt(peb))
	{
		...
		while (first != query)
		{
			if (!m()->chkvirt(query) || !m()->chkvirt(((PUCHAR)query + sizeof(LDR_DATA_TABLE_ENTRY) - 1)))
				return 0;
			...
			if (Name && m()->chkvirt(Name))
			{
				...
				if (NameBuffer && m()->chkvirt(NameBuffer) && NameLength && m()->chkvirt((NameLength + ((PUCHAR)NameBuffer) - 1)))
				{
					WCHAR wName[80];
					...
					if (m()->read_wstring((u64)NameBuffer, (pwstr)wName, ReadLen))
					{
						...
						mod_cnt++;
						if (!callback((u64)r(data->DllBase), r(data->SizeOfImage), (pwstr)sufMooduleName))
						{
							return mod_cnt;
						}
					}
...
int process::enummod32(std::function<bool(u64 base, u64 size, pwstr name)> callback)
{
	...
	if (peb && m_mmu->chkvirt(peb))
	{
		...
		while (first != query)
		{
			if (!m()->chkvirt(query) || !m()->chkvirt(((PUCHAR)query + sizeof(LDR_DATA_TABLE_ENTRY32) - 1)))
				return 0;
			...
			if (Name && m()->chkvirt(Name))
			{
				...
				USHORT NameLength = r(Name->Length);
				if (NameBuffer && m()->chkvirt((PVOID)NameBuffer) && NameLength && m()->chkvirt((NameLength + ((PUCHAR)NameBuffer) - 1)))
				{
					WCHAR wName[80];
...
int process::enumvad(std::function<bool(u64 base, u64 size, u64 prot, u64 type)> callback)
{
	// int VadRoot;
	auto pdb = sys()->allpdbs();
	if (pdb != nullptr)
	{
		...
		int StartingVpnHigh = ntos.structures["_MMVAD_SHORT"]["StartingVpnHigh"].offset;
		...
		u64 MmProtectToValue = sys()->GetNtoskrnl() + ntos.symbols["MmProtectToValue"];
		...
		BalancedNodeTraverse(FirstNode, [&](u64 Node) -> bool
			...
			int VadType=GetBitFieldValue(Flags,r_ctx->VadType.bitfield_offset,r_ctx->VadType.bitfield_offset+r_ctx->VadType.bitfield_length-1);
			int Protection=GetBitFieldValue(Flags,r_ctx->Protection.bitfield_offset,r_ctx->Protection.bitfield_offset+r_ctx->Protection.bitfield_length-1);
			int PrivateMemory=GetBitFieldValue(Flags,r_ctx->PrivateMemory.bitfield_offset,r_ctx->PrivateMemory.bitfield_offset+r_ctx->PrivateMemory.bitfield_length-1);

			u64 StartingVirtualAddress = StartingVpn | ((static_cast<u64>(StartingVpnHigh))<<32);
			EndingVpn+=1;
			u64 EndingVirtualAddress = EndingVpn | ((static_cast<u64>(EndingVpnHigh))<<32);
...
std::list<std::tuple<u64, u64, u64, u64>>
process::traversevad()
{
...
std::list<std::tuple<u64, u64, std::string>>
process::traversemod()
{
	...
	enummod([&](u64 base, u64 size, pwstr name) -> bool
		...
		p1x(name);
		r.push_back
			(std::make_tuple(base,size,converter.to_bytes(name)));
		return true; });
	return std::move(r);
}
std::list<std::tuple<u64, u64, std::string>>
process::traversemod32()
{
	std::list<std::tuple<u64, u64, std::string>> r;
...
u64 process::modbase(const char *name)
{
	...
	u64 result = 0;
	enummod(
		[&](u64 base, u64 size, pwstr name) -> bool
		{
