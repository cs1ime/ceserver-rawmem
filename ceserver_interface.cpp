...
namespace ceserver
{
    ...
    public:
        ...
        uint64_t GetProcessObject() const
        {
    ...
    int ReadProcessMemory(HANDLE hProcess, uint64_t lpAddress, void *buffer, int size)
    {
        if (GetHandleType(hProcess) == htProcesHandle)
        {
            ...
            if (p)
            {
                return ceserver_impl::ReadProcessMemory(p->GetProcessObject(), lpAddress, buffer, size);
            }
        }
        return 0;
    }
    int WriteProcessMemory(HANDLE hProcess, uint64_t lpAddress, void *buffer, int size)
    {
        if (GetHandleType(hProcess) == htProcesHandle)
        {
            ...
            if (p)
            {
                return ceserver_impl::WriteProcessMemory(p->GetProcessObject(), lpAddress, buffer, size);
            }
        }
        return 0;
    }
    ...
    std::list<region_info> TraverseMemoryRegion(uint64_t pid)
    {
        ...
        result.sort([](auto a, auto b) -> bool
    ...
    std::map<int, std::pair<uint64_t, std::list<region_info>>> m_VirtualQueryEx_Cache;
    std::mutex m_VirtualQueryEx_Cache_lock;
    int VirtualQueryExImpl(Process *p, uint64_t lpAddress, region_info *rinfo, char *mapsline)
    {
        auto pid = p->GetPid();
    ...
    VirtualQueryExFull(HANDLE hProcess, uint32_t flags)
    {
        if (GetHandleType(hProcess) == htProcesHandle)
        {
            ...
            if (p)
            {
                return std::make_shared<std::list<region_info>>(TraverseMemoryRegion(p->GetPid()));
            }
        }
